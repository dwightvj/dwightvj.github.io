...
In this blog post, I explore *spectral clustering* which is an algorithm that allows us to flexibly cluster data with no preconceptions about how it might be strucured or formed. This is unlike traditional clustering (i.e. K-Means) which assumes that points in a respective cluster are spherically distributed around a respective cluster's centers.
...
Let us use the distance between points to create clusters. In doing so we can create a matrix with shape `(n, n)` such t`n` is the number of data points. This becomes our similarity matrix. Some aspects of this similarity matrix include that each entry  `A[i,j]` should be equal to `1` if `X[i]` is within distance of some specified `epsilon` of `X[j]`. On top of this, because we don't want to compare points to each other **the diagonal entries `A[i,i]` should all be equal to zero.**

We will use use `epsilon = 0.4` for the sake of consistency.

{::options parse_block_html="true" /}
<div class="gave-help">
I recommended to my peers to use pairwise_distances from sklearn to compute all pairwise distances in a more efficient fashion. By doing so they are able to avoid manually searching to see whether `(X[i] - X[j])**2 < epsilon**2` in a for loop for example.
</div>
...
{::options parse_block_html="true" /}
<div class="got-help">
My peers recommended I take the square of epsilon when in order to create better demarcated clusters down the line.
```python
# initialize epsilon
epsilon = 0.4
...
The 2nd part of thte norm cut objective is what is called the *volume term*. The volume term allows us to measure how big clusters. In order to do so we compute the degree in a row - which in turn tells us the number of connections across points. In doing so we are able to then compute the sum for degrees corresponding to a particular cluster (i.e. $$C_0$$ and $$C_1$$).
...
Let us construct the matrix $$\mathbf{L} = \mathbf{D}^{-1}(\mathbf{D} - \mathbf{A})$$ (i.e. the *Laplacian matrix*), and find the eigenvector corresponding to its second-smallest eigenvalue. 
...
Additionally let us demonstrate how successful this new function is on the data from earlier by running this function on different sets of data

{::options parse_block_html="true" /}
<div class="got-help">
My peers reminded me to cut down this function to at most 10 lines of code for the sake of efficiency. I was able to do so by combining many operations especially when constructing my similarity matrix as seen below.
```python
def spectral_clustering(X, epsilon):
    ...
    # let those valus such that z_eig > 0 be 1, and z_eig < 0 be 0
    z_eig[z_eig > 0] = 1
